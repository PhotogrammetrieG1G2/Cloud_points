...
def obtain_correspondances():
    ...
    image_paths = [os.path.join(image_folder, f) for f in sorted(os.listdir(image_folder)) if f.lower().endswith(".png")]
    ...
    # Détection des points clés (SIFT normal + dense)
    for img in images:
        ...
        des = np.vstack((des_normal, des_dense))
        keypoints.append(kp)
